...
import { getPost, updatePost } from '../api-helpers/helper';
...
const DiaryUpdate = () => {
    ...
    useEffect(()=>{
        getPost(id).then(data=>
            {setPost({title:data.post.title,location:data.post.location,image:data.post.image,description:data.post.description});}
    ...
    const handleSubmit=(e)=>{
        e.preventDefault();
        console.log(post);
        updatePost(id,post).then(data=>console.log("Updated post")).catch(err=>console.log(err));
    }
    ...
    >
        ...
        <form onSubmit={handleSubmit}>
            ...
            >
                ...
                <TextField disabled type={"date"} name="date" value={post.date} onChange={handleChange} variant='standard'/>
